...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/vanoHW4db"
...
class User(UserMixin, db.Model):
    ...
    user_collection = db.relationship('PersonalGifCollection',backref='user')
...
class Gif(db.Model):
    ...
    def __repr__(self):
        return "Name: {} (URL: {})".format(self.title,self.embedURL)
...
#  364: The following forms for searching for gifs and creating collections are provided and should not be edited. You SHOULD examine them so you understand what data they pass along and can investigate as you build your view functions in TODOs below.
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    # 364: This function should get or create a personal gif collection. Uniqueness of the gif collection should be determined by the name of the collection and the id of the logged in user.
    ...
    # fetch a collection using the passed parameters (will return None if it doesn't exits)
...
def index():
    # 364: Edit this view function, which has a provided return statement, so that the GifSearchForm can be rendered.
    form = GifSearchForm()
...
def search_terms():
    # 364: Edit this view function so it renders search_terms.html.
...
def create_collection():
    ...
    if request.method == 'POST':
        ...
        gif_list = []
        for gif_id in gif_picks:
            g = get_gif_by_id(gif_id)
            gif_list.append(g)
...
def collections():
    ...
    # 364: This view function should render the collections.html template so that only the current user's personal gif collection links will render in that template. Make sure to examine the template so that you send it the correct data!
